...
import { houses as housesArr, lands as landsArr } from "../../db/openDb";
...
const HomeScreen = ({ navigation }) => {
  ...
  const fetchData = () => {
    getAllHouses(housesArr, setHouses);
    getAllLands(landsArr, setLands);
    getMostRecentHouses(housesArr, setNewestHouses);
    getMostRecentLands(landsArr, setNewestLands);
  };
  useEffect(() => {
    fetchData();
  }, []);
  ...
  const ListCategories = () => {
    const categoryList = ["تازەترین", "هەرزانترین", "گرانترین"];
  ...
  return (
    <SafeAreaView style={{ backgroundColor: COLORS.white, flex: 1 }}>
      ...
      <View style={style.header}>
        ...
        <Icon onPress={fetchData} name="refresh" size={30} />
      </View>
      <ScrollView>
        <View
          style={{
        ...
        <ListOptions />
        <ListCategories />
        {typeof whichCardsToShow() == "object" ? (
          <FlatList
            ...
            renderItem={({ item }) => (
              <ListCards navigation={navigation} house={item} />
            )}
          />
        ) : (
          <Text
